...
interface WishlistItemProps {
  favourites: any[];
  handleRemoveFavourite: (wishListID: string) => void;
}

const WishlistItem = ({ favourites, handleRemoveFavourite }: WishlistItemProps) => {
  const { t } = useTranslation();
  const { currentCurrency } = useSelector((state) => state.app);

  const navigate = useNavigate();

  const handleClickWishlistItem = (item: any) => {

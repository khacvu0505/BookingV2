...
const StickyHeader = ({ offCanvasRef, services }) => {
  ...
  const handleChooseRoom = () => {
    // eslint-disable-next-line no-undef
    const roomElement = document.getElementById("tour-package");
  ...
  return (
    <div className={`singleMenu js-singleMenu ${header ? "-is-active" : ""}`}>
            ...
            <div className="col-auto">
              <div className="row x-gap-15 y-gap-15 items-center">
                  ...
                  <div className="text-14">
                    ...
                    <span className="text-22 text-dark-1 fw-500 ml-10">
                      {services?.length > 0
                        ? formatCurrency(
                            getMinValueFromList(
                              services?.map((item) => item?.finalPrice)
                            )
                          )
                        : 0}{" "}
                      {currentCurrency}
                ...
                <div className="col-auto">
                  ...
                  >
                    Chọn tour <div className="icon-arrow-top-right ml-15" />
                  </button>
                </div>
              </div>
            </div>
      ...
      <OffCanvasDetail ref={offCanvasRef} />
    </div>
  );
};
